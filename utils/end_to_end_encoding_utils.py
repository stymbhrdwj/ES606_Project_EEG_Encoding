def load_images(args, idx_val):
	...
	img_dirs = os.path.join(args.project_dir, 'stimuli', 'training_images')
	image_list = []
	...
	img_dirs = os.path.join(args.project_dir, 'stimuli', 'test_images')
	image_list = []
	for root, dirs, files in os.walk(img_dirs):
		for file in files:
			if file.endswith(".jpg"):
...
def load_eeg_data(sub, args, idx_val):
	...
	data_dir = os.path.join('preprocessed_data', 'sub-'+format(sub,'02'))
	training_file = 'preprocessed_eeg_training.npy'
	data = np.load(os.path.join(args.project_dir, data_dir, training_file), allow_pickle=True).item()
	...
	data = np.load(os.path.join(args.project_dir, data_dir, test_file), allow_pickle=True).item()
